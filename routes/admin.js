...
router.get("/get_My_Orders_Request", (req, res) => {
  ...
  let Query =
    "SELECT `id`, `grandtotal`, `paymentmethod`, `city`, `zipcode`, `address`, `remarks`, `status`, `trackingid`, `companyname`, `trackingurl`, `trackremarks`, `personreceived`, `returnremarks`, `username`, `refid`, date_format(`datetime`,'%Y-%m-%dT%T') as datetime FROM `bill` order by id desc";
...
router.post("/stockRequest", (req, res) => {
  ...
  // VIEW Stock Request Handler
    ...
    Query =
      "SELECT product.productid,product.productname,product.pdescription,stocks.qty,stocks.dateofstock,stocks.stockid FROM stocks INNER JOIN product on product.productid=stocks.productid ORDER BY `stockid` DESC";
...
router.post("/productRequest", (req, res) => {
  ...
  // VIEW Product Request Handler
    ...
    Query =
      "SELECT * FROM `product` INNER JOIN subcategory ON product.subcategoryid=subcategory.subcategoryid ORDER BY `productid` DESC";
    conn.query(Query, (err, rows) => {
      if (err) throw err;
  ...
  // SESSION Request Handler
    ...
    if (session.username != undefined) {
      ...
      res.send(""); // --* Client ko USERNAME ki value jayegi nahi toh Empty("") jayegi *--
    }
  }
  // Get SUB-CATEGORY
...
router.post("/subcategoryRequest", (req, res) => {
  ...
  // VIEW Category Request Handler
    ...
    Query =
      "select * from subcategory inner join category on subcategory.categoryid=category.categoryid ORDER BY `subcategoryid` DESC";
