...
router.post("/searchProductAction", (req, res) => {
  ...
  if (req.body.category === undefined) {
    ...
    let Query =
      "SELECT * FROM `product` INNER JOIN subcategory on product.subcategoryid=subcategory.subcategoryid where subcategory.categoryid=" +
      categoryid;
    console.log(Query);
    conn.query(Query, function (err, rows) {
      if (err) throw err;
...
router.get("/getMenSubcategoryAction", (req, res) => {
  let name = "men";
  let Query =
    "SELECT subcategory.subcategoryid,subcategory.subcategoryname,category.categoryname FROM `subcategory` INNER JOIN category ON subcategory.categoryid=category.categoryid WHERE category.categoryname='" +
    name +
    "'";
  conn.query(Query, (err, rows) => {
...
router.post("/userUpdatePassword", (req, res) => {
  ...
  let passwordQuery =
    "select * from users_2 where username='" +
    username +
    "' and password='" +
    oldpassword +
    "'";
...
router.get("/get_Cancelled_Orders_Request", (req, res) => {
  ...
  let Query =
    "SELECT `id`, `grandtotal`, `paymentmethod`, `city`, `zipcode`, `address`, `remarks`, `status`, `trackingid`, `companyname`, `trackingurl`, `trackremarks`, `personreceived`, `returnremarks`, `username`, `refid`, date_format(`datetime`,'%Y-%m-%dT%T') as datetime FROM `bill` where username='" +
    username +
...
router.post("/cancel_Order_request", (req, res) => {
  ...
  let Query =
    "UPDATE `bill` SET `status`='cancelled' WHERE `id`='" + cancelid + "'";
  console.log(Query);
...
router.post("/get_My_OrdersDetails_Request", (req, res) => {
  ...
  let Query =
    "SELECT bill_detail.id,bill_detail.price,bill_detail.discount,bill_detail.netprice,bill_detail.quantity,product.productname,product.photo,product.pdescription FROM bill_detail INNER JOIN product ON bill_detail.productid=product.productid WHERE bill_detail.billid='" +
    id +
    "'";
  conn.query(Query, function (err, rows) {
    if (err) throw err;
...
router.get("/get_My_Orders_Request", (req, res) => {
  ...
  // let Query = "SELECT `id`, `grandtotal`, `paymentmethod`, `city`, `zipcode`, `address`, `remarks`, `status`, `trackingid`, `companyname`, `trackingurl`, `trackremarks`, `personreceived`, `returnremarks`, `username`, `refid`, date_format(`datetime`,'%Y-%m-%dT%T') as datetime FROM `bill` where username='" + username + "'";
  let Query =
    "SELECT `id`, `grandtotal`, `paymentmethod`, `city`, `zipcode`, `address`, `remarks`, `status`, `trackingid`, `companyname`, `trackingurl`, `trackremarks`, `personreceived`, `returnremarks`, `username`, `refid`, date_format(`datetime`,'%Y-%m-%dT%T') as datetime FROM `bill` where username='" +
    username +
    "' AND NOT status='cancelled' order by id desc";
...
function getcurrentstock(productid) {
  return new Promise((resolve, reject) => {
    ...
    let Query =
      "SELECT ifnull( sum(qty),0) as totalstock from stocks where  productid='" +
      productid +
      "'";
    console.log(Query);
    conn.query(Query, function (err, rows) {
      ...
      let Query2 =
        "SELECT ifnull( sum(quantity),0) as totalsoldstock from `bill_detail` where  productid='" +
        productid +
        "'";
      console.log(Query2);
...
router.post("/addToCartSelectedProduct", async (req, res) => {
  ...
  if (action == "add") {
    ...
    for (let i = 0; i < cart.length; i++) {
      // CART ke jis-jis Product ki ID, Remove vale product_id se match kar rahi, uske elava baaki Products ko TEMPCART me Push karde
...
router.post("/paymetRequest", (req, res) => {
  ...
  if (session.user_username == undefined) {
    ...
    let subQuery = "select * from users_2 where username='" + username + "'";
    conn.query(subQuery, (err, rows) => {
      if (err) throw err;

      // if Username found from users_2 Table
      if (rows.length > 0) {
        let email = rows[0].email;

        // Now generate message to send USER.
        // let message =
        //   "Your order is placed at " +
        //   datetime +
        //   ". You will receive your order within 5-6 working days. Thankyou for shopping with us.";
        // console.log(message);
        ...
        conn.query(Query, (err, result, fields) => {
          ...
          for (let i = 0; i < cart.length; i++) {
            ...
            let detailsQuery =
              "INSERT INTO `bill_detail`(`id`, `price`, `discount`, `netprice`, `quantity`, `productid`, `billid`)" +
              " VALUES (null," +
              discountedPrice +
              "," +
              discount +
          ...
          // Mail Details
          //   let mailDetails = {
          //     from: "subhratocoder@gmail.com",
          //     to: email,
          //     subject: "Thank you " + username + " - Apparel Shopping",
          //     text: "" + message + "",
          //   };
...
router.post("/userSignupRequest", (req, res) => {
  ...
  var selectQuery = "select * from users_2 where username='" + username + "'";
